...
- 2026-09-09: the public site and brand artwork were re-based onto the X identity (deep blue-black ground, off-white geometric XI, blue pitch centre circle) with locally hosted Inter replacing the serif/sans mix. Presentation only — same pages, same figures, same frozen settlement semantics; brand assets are regenerated deterministically by `scripts/brand-kit.mjs` and `scripts/build-brand-kit.mjs` (reviewable kit in `brand/`), and the share card moved to `og-image-v2.png` to bust external caches. Site X links point at `https://x.com/patternxi_xi`.
- 2026-09-10: presentation polish — the SVG masters now set an explicit off-white fill on the XI letters (standalone SVG loads previously fell back to black), the wordmark embeds real Inter outlines instead of live text, KPI sparklines render only from real ledger series, the empty equity curve shows a clean zero-line empty state, and the 52-golden-case engineering stat moved from the homepage KPI strip to the verification page. No settlement, evidence or distribution change.
- 2026-09-10: the sticky masthead background is now fully opaque (#101820) so scrolled content no longer shows through.
