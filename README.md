...
2. **每日 manifest + OpenTimestamps**——CI 每天把当日所有推介文件的 SHA-256 清单锚定到比特币区块链（`ots verify` 永久可验，无需信任任何运营方）；清单之间首尾哈希相连形成链；
...
manifests/YYYY-MM-DD.txt    每日清单：当日推介哈希 + 前日清单哈希（哈希链）
manifests/*.ots             OpenTimestamps 比特币锚定回执
...
`npm run validate` → PR 合并。CI 用 GitHub 服务器时钟强制 2 小时规则。
...
#    - Require 1 approval（自荐评审 = 自我摩擦，替代原项目的操作员边界）
#    - 勾选包括 Require status checks: Check / Ledger integrity
```

之后 CI 全自动：PR 校验、每日锚定、站点部署。
